...
#### ทดสอบทุกคำสั่งเสียงแบบพิมพ์ (งาน 33, 2026-09-25)

- **ออฟไลน์กับโค้ดล่าสุด** (`tools/voice/gen_matrix.py` + `full_route.py`): 81/81 แถวไปทางที่คาด · full route 80/80 (D90 ต้องมีร่างนัดค้าง) · ก่อนแก้ `lights.NOT_LIGHTS` มี 4 แถวผิด: "เปิดเพรงของคาราบาว" "เปิดคลิปแมว" "เปิดวิทยุ" "เปิดไฟฉาย" ไปทางไฟ
- **บนมือถือ แถว SAFE 47 แถว** (`TEST_ASK`, broker บน VPS ที่ยังไม่ได้ deploy รอบนี้): คำสั่งในโค้ดถูกทุกแถว — ปลุก/นัดถามกลับเวลากำกวม, โน้ตอ่านได้ และ "ลบนม…" ตอบว่ายังทำด้วยเสียงไม่ได้, ไฟอ่านสถานะ 1 จาก 4, เพลง/วิดีโอ play-pause-next-prev-louder-quieter-stop, อากาศ ฝุ่น เวลา วันที่ ดีเซล ข้อเท็จจริง มฟล. ถูก
- **คำตอบของโมเดลที่ผิด (prompt เก่ากว่าแอป):** "ตั้งปลุกยังไง" → "ผมตั้งปลุกไม่ได้" · "เปิดวิทยุ" / "จับเวลาห้านาที" / "อัดเสียงให้หน่อย" → "ใช้แอปในโทรศัพท์เอง" (มีแอปในแผงควบคุมแล้ว) · "เล่านิทานสั้นๆ" และ "กล้องวงจรปิดยี่ห้อไหนดี" ถูกปฏิเสธ · "ราคาทองวันนี้" → ดูไม่ได้ (ราคาทองอยู่บนจอ แต่ไม่อยู่ใน prompt) — รอ Poom ตัดสินการแก้ prompt
- แถวที่ไม่ปลอดภัย (ตั้งปลุกจริง ลงนัด สั่งไฟ เพิ่มโน้ต กล้อง แผนที่) ดูผลออฟไลน์เท่านั้น ยกเว้นที่ทำในการทดสอบรวม (ข้อ 34)

#### เทียบ STT: Groq whisper-large-v3-turbo (+คำใบ้) กับ Qwen qwen3-asr-flash (งาน 33, A07 2026-09-25)

`tools/voice/run_stt_clips.sh` → `/v1/stt` ของ broker จริงผ่าน `TEST_STT_FILE` · 76 ประโยค × 2 เสียง edge-tts (Premwadee หญิง, Niwat ชาย) = 152 คลิป 255 วินาที · ให้คะแนนด้วย `score.py` เฉพาะคลิปที่ทั้งสองตัวตอบ (118 คลิป)

| | ตรงทั้งประโยค | ส่วนสำคัญถูก | CER | median / p90 (รวม am ~300ms) | ด่าน speech gate ตัด |
|---|---|---|---|---|---|
| groq-hints | 25.6% | 33.1% | 38.3% | 640 / 919 ms | 26 |
| qwen | 86.4% | 94.1% | 2.4% | 736 / 935 ms | 0 |

- จับคู่คลิปเดียวกัน: ส่วนสำคัญถูกเฉพาะ Qwen 73 คลิป เฉพาะ Groq 2 คลิป (D03 ทั้งสองเสียง: Qwen ได้ยิน "ลุงนัด")
- Groq แย่มากกับเสียงชาย (Niwat ตรงทั้งประโยค 3.9%) · ใช้ Qwen ไปราว 246 วินาที
- ⚠ **31 คลิปของ Groq ล้มเพราะเพดานของ broker เอง** (`rate_limited` 10/นาที แล้ว `rate_limited_daily` 300 ครั้ง/วัน ต่อ endpoint) ไม่ใช่ Groq — รอบนี้ใช้เพดาน STT ของวันจนหมด จาร์วิสถอดเสียงไม่ได้ถึงเที่ยงคืน (เวลาไทย) · ครั้งหน้า: คลิป × ผู้ให้บริการ ต้องต่ำกว่า 300 ต่อวันมาก และ GAP ≥ 7 วินาทีเมื่อสลับสองตัว · probe log `status= code=` แล้ว
- ข้อจำกัด: เสียงสังเคราะห์ ไม่ใช่เสียง Poom · ไม่มีเสียงจริงจากโหมดวิเคราะห์ให้เทียบ (ไม่เก็บเสียง)

#### ทดสอบทั้งตู้บน A07 (ข้อ 34, 2026-09-25)

เพดาน STT ของ broker หมดจากการเทียบ STT จึงทำคำสั่งเป็น**ข้อความพิมพ์** (`TEST_ASK`) แทนเสียงสังเคราะห์ ยกเว้นคำปลุก (ตรวจแล้ว 4/4 หน้าแอป) · เสียงจริงหนึ่งตาได้ `stt failed: rate_limited_daily (429)` และจาร์วิสพูดข้อความของ broker ไม่ค้าง

| ขั้น | ผล |
|---|---|
| "เพิ่มนมในรายการซื้อของ" → "อ่านรายการซื้อของ" → เอาออกในแอปโน้ต | ✅ `note_add:shopping:ok` "เพิ่ม นม ในรายการซื้อของแล้วครับ" → "รายการซื้อของมี 1 อย่างครับ นม" → เอาออก (ไม่ต้องยืนยันตัวตน) → อ่านซ้ำ 28 ตัวอักษร = ว่าง |
| ลงนัดถึงขั้นยืนยันตัวตนแล้วยกเลิก | ✅ `verify_identity:asked` → หน้ายืนยันตัวตน (ไม่แคปจอ) → "ยกเลิก" → `KioskAuth outcome=cancelled` ไม่เคยส่ง "ยืนยัน" · 🔶 ไม่ได้เปิด Google Calendar ดูเอง · การอ่านนัดก็ขอยืนยันตัวตนเหมือนกัน |
| นับถอยหลัง 3 นาที จอดับ (`keyevent 223`) | ✅ เริ่ม 11:47:21.807 ดัง 11:50:21.846 (`countdown ended source=alarm set_s=180`) จอตื่นเอง "หยุดเสียง" บนจอ |
| วิทยุ + ปุ่มจาร์วิสของหน้าวิทยุ | ✅ `ducked for jarvis volume=0.15` → ไม่มีคนพูด ยกเลิก → `volume back after jarvis volume=1.0` |
