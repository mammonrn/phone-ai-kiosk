...
`weather` `gold` `crypto` **แต่ละก้อนล้มแยกกัน** ก้อนที่ล้มส่ง `ok: false`
...
### ย้ายบ้าน / เปลี่ยนเมืองที่ดูอากาศ
...
```json
{"weather_latitude": 20.4342, "weather_longitude": 99.8836, "weather_place": "แม่สาย เชียงราย"}
```

`weather_place` เป็นแค่ข้อความบนหัวหน้าต่าง ไม่ได้ใช้ค้นพิกัด — ตั้งพิกัดกับ
ชื่อให้ตรงกันเองด้วย แก้แล้ว `sudo systemctl restart kiosk-broker`
