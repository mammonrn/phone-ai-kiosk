...
### ถอดซ้ำคำสั่งแผนที่ด้วย Qwen (0.51.0, Poom ตัดสิน 2026-09-24)

จากเสียงของ Poom 16 ประโยค: qwen ได้ชื่อสถานที่ถูกทุกครั้ง (ภูชี้ฟ้า ดอยตุง สิงห์ปาร์ค วัดร่องขุ่น)
ส่วนคำสั่งไฟ กล้อง ปลุก ได้เหมือนกันทั้งสองตัว จึงทำแบบนี้ (`maps_rescue.py`):
1. `groq-hints` ยังถอดทุกประโยคเหมือนเดิม
2. ถ้าสิ่งที่ Groq ได้มีคำแผนที่ (นำทาง แผนที่ พาไป ไปที่ เส้นทาง) broker ส่งเสียงชุดเดิมให้ Qwen อีกครั้ง
   แล้วใช้ข้อความของ Qwen **เฉพาะเมื่อข้อความนั้นยังเป็นคำสั่งแผนที่** (Qwen เปลี่ยนชื่อที่ได้ แต่เปลี่ยนคำสั่งเป็นอย่างอื่นไม่ได้)
3. Qwen ผิดพลาด ช้ากว่า `maps_rescue_timeout_s` (2.5 วินาที) ไม่มีคีย์ หรือวินาทีฟรีไม่พอ → ใช้ข้อความของ Groq ตามเดิม
4. ตัวเลขที่ Qwen เขียนเป็นคำ ("สิบเอ็ดโมง") เปลี่ยนเป็นเลข ("11 โมง") ก่อนใช้ เฉพาะที่เป็นจำนวนจริง
   (ตามด้วยหน่วย เช่น โมง ทุ่ม นาที หรือหลังคำอย่าง ตี บ่าย ซอย หมู่) ชื่อที่มีคำเลขอยู่ข้างในไม่ถูกแตะ:
   วัดเจ็ดยอด ห้าแยกพ่อขุน สามเหลี่ยมทองคำ ห้าง ร้อยเอ็ด (`thai_numbers.py`)

**คำสั่งอื่นไม่ช้าลง:** ประโยคที่ไม่มีคำแผนที่ทำแค่ค้นคำ 5 คำในข้อความ ไม่อ่านไฟล์คีย์ ไม่แตะฐานข้อมูล ไม่เรียกเครือข่าย
และถ้าตัวถอดเสียงที่ใช้ไม่ใช่ `groq-hints` (เช่นสลับจาก adb) จะไม่ถอดซ้ำ

**ความเป็นส่วนตัว:** ตั้งแต่รุ่นนี้ เสียงของ **คำสั่งแผนที่** ถูกส่งไป Alibaba Cloud สิงคโปร์ด้วยโดยค่าเริ่มต้น
(คำสั่งอื่นไปที่ Groq เท่านั้นเหมือนเดิม) ปิดได้ด้วย `"maps_rescue": false` ใน `config.json` แล้ว restart

**เงิน:** ถอดซ้ำ **เฉพาะเมื่อวินาทีฟรีที่เหลือพอสำหรับเสียงทั้งคลิป** และไม่เกิน 23 ธ.ค. 2026 จึงไม่มีวันจ่ายเงิน
- log เตือนที่ 80% (`free tier 80% used: Qwen ASR ...`)
- เมื่อไม่พอแล้ว หยุดเอง และเขียน `maps rescue stopped: Qwen's free seconds are used up` (ครั้งเดียวต่อการเริ่ม broker)
- ถ้า Alibaba ตอบว่าโควตาหมด (code `...FreeTierOnly` / `Arrearage`) ก็หยุดจนกว่าจะ restart
- ครั้งที่ Qwen ช้าเกินเวลา นับวินาทีนั้นว่าใช้ไปแล้วด้วย (อาจถูกคิดจริง) นับขาดดีกว่านับเกิน
- `$B usage` มีบรรทัด `maps rescue (Qwen hears map commands too): on / STOPPED — ...`
- ⚠️ ตัวนับเป็นของ broker เอง ถ้าใช้คีย์เดียวกันที่อื่นด้วย ตัวเลขจะต่างจาก console แนะนำเปิด
  "Free quota only / Stop on exhaust" ใน Model Studio console เป็นอีกชั้น (ถ้ามีให้เปิด)

**log:** ทุกบรรทัด `stt ok ...` มี `provider=groq-hints text_from=<groq-hints|qwen> rescue=<ผล> ms=<Groq> rescue_ms=<ที่รอ Qwen เพิ่ม>`
- `rescue=used` ใช้ของ Qwen และต่างจาก Groq · `same` ได้เหมือนกัน · `-` ไม่ใช่คำสั่งแผนที่
- `fallback-timeout / -error / -rate-limit / -auth / -empty / -not-maps / -vendor-quota` ถาม Qwen แล้วใช้ของ Groq
- `off-quota / off-expired / off-no-key / off-vendor-quota` ไม่ได้ถาม Qwen
```bash
sudo journalctl -u kiosk-broker --since today | grep -o "rescue=[a-z-]*" | sort | uniq -c
sudo journalctl -u kiosk-broker --since today | grep "rescue=used\|rescue=same" | grep -o "rescue_ms=[0-9]*"
```
`$B analysis summary` แสดงตัวถอดเสียงเป็น `groq-hints+qwen` สำหรับรอบที่ใช้ของ Qwen

**คำใบ้เฉพาะ Qwen:** `stt_hints.json` มีช่องใหม่ `"qwen_phrases"` ส่งให้ Qwen เท่านั้น ต่อจาก `phrases`
รวมไม่เกิน 400 ตัวอักษร Groq ยังได้แค่ 150 ตัวแรกของ `phrases` เหมือนเดิม **ไม่เปลี่ยนเลยแม้แต่ตัวเดียว**
**Poom อนุมัติแล้ว 2026-09-24** อยู่ใน `server/stt_hints.json` ของ repo: ชื่อสถานที่ 22 ชื่อ และชื่อไฟจริง
ไฟหน้าบ้าน ไฟด้านหน้า (Qwen 386 จาก 400 ตัวอักษร) `install.sh` **ไม่เขียนทับ** ไฟล์บน VPS จึงต้องคัดลอกเองหนึ่งครั้ง
**ก่อนคัดลอก** รัน `$B stt-hints-check` ถ้า whisper prompt บน VPS ต่างจาก
"จาร์วิส กล้อง ขอดูกล้อง เปิดกล้อง ดูกล้อง ตั้งปลุก ปลุก แผนที่ พาไป เซ็นทรัล เซ็นทรัลเชียงราย เชียงราย อากาศ ราคาทอง แม่ฟ้าหลวง ไฟ"
แปลว่าไฟล์บน VPS ถูกแก้ไว้ ให้เพิ่มแค่ `"qwen_phrases"` เข้าไฟล์เดิมแทนการคัดลอกทับ:
```bash
cd ~/phone-ai-kiosk && git pull
sudo install -o kioskbroker -g kioskbroker -m 0644 server/stt_hints.json /home/kioskbroker/.config/kiosk-broker/stt_hints.json
$B stt-hints-check     # whisper prompt ต้องเหมือนเดิม 130 ตัว · qwen context ≤ 400 และไม่มี NOTE ว่าตัดคำ
```

**ทดสอบด้วยเสียงจริงที่เก็บไว้ (ไม่เกิน $0.05 ต่อรอบ ราคาเต็ม):**
```bash
$B analysis summary                                   # จด #id ของแถวที่มี audio
$B stt-compare --ids <id ทั้งหมด> --providers groq-hints,rescue --maps-check
```
- แถว `rescue` บอก `<used +412 ms qwen>` ฯลฯ คือสิ่งที่ /v1/stt ส่งให้มือถือจริง
- บรรทัด `rescue: N turns, M sent to Qwen ...; extra wait mean X ms` คือคำสั่งแผนที่ช้าขึ้นเท่าไร
- ประโยคที่ไม่ใช่แผนที่ต้องขึ้น `<->` และข้อความเหมือนแถว `groq-hints` ทุกตัวอักษร
- `--maps-check` ถามโมเดลแบบเดียวกับ /v1/chat สำหรับทุกข้อความแผนที่ (ทั้งของ Groq และของ rescue)
  แล้วพิมพ์ `-> Maps: <ที่ที่จะเปิด>` ราว $0.0015 ต่อคำถาม นับรวมในเพดาน `--max-usd`
...
ตั้งแต่ 0.46.0 **สั่งเปิดปิดได้เฉพาะอุปกรณ์ที่อยู่ใน allowlist** (ด้วยเสียงหรือกดบนการ์ด)
คำสั่งไปถึง eWeLink ได้ทางเดียวคือ `ewelink.send()` ใน broker (มี test ตรวจ) และผ่านด่านในโค้ดทุกครั้ง:
สวิตช์หยุดฉุกเฉิน → จำกัด 10 ครั้งต่อนาที / 300 ครั้งต่อวัน → allowlist ตาม id → ห้ามอุปกรณ์ต้องห้าม
(กล้อง ประตู กุญแจ โรงรถ รั้ว ม่าน สัญญาณเตือน ตาม uiid และชื่อ) → ต้องเป็นไฟ สวิตช์ไฟ หรือปลั๊ก
(ปลั๊กได้เพราะ Poom อนุมัติ 2026-09-24 ให้ Light1 และ Light2) → เปิดหรือปิดเท่านั้น
...
### ขั้นที่ 5 — อนุญาตให้สั่งเปิดปิด (0.46.0)

ยังไม่มีอะไรสั่งได้จนกว่าจะเพิ่มเข้า allowlist ไม่มีไฟล์ config ให้คัดลอกเอง คำสั่งด้านล่างสร้างไฟล์
`ewelink_allowlist.json` และ `ewelink_names.json` ใน `~/.config/kiosk-broker/` ให้เอง (0600)

```bash
$B ewelink-devices                 # ดู …4 ตัวท้ายของ id และชื่อแต่ละช่องของสวิตช์ 4 ช่อง
$B ewelink-allow …xxxx             # Light1 (ปลั๊ก)          ใส่ 4 ตัวท้ายจริงแทน xxxx
$B ewelink-allow …yyyy             # Light2 (ปลั๊ก)
$B ewelink-allow …zzzz             # Switch1 ทุกช่อง  (หรือ  $B ewelink-allow …zzzz 1,2  เฉพาะช่อง 1 และ 2)
$B ewelink-allowlist               # ต้องขึ้น switching: on และ 3 อุปกรณ์
```

**ชื่อช่องของสวิตช์ 4 ช่อง:** `ewelink-devices` แสดงชื่อแต่ละช่องพร้อมที่มา
- `(ชื่อจาก eWeLink)` = ชื่อที่ตั้งในแอป eWeLink ส่งมาทาง API แล้ว ใช้ได้เลย
- `(ยังไม่มีชื่อ)` = API ไม่ได้ส่งชื่อมา ให้ตั้งชื่อของเราเอง (ใช้แทนชื่อจาก eWeLink เสมอ):

```bash
$B ewelink-name …zzzz 1 "ไฟหน้าบ้าน"   # ช่อง 1
$B ewelink-name …zzzz - "สวิตช์ห้องนั่งเล่น"   # ชื่อของตัวอุปกรณ์เอง
$B ewelink-name …zzzz 1 -              # ลบชื่อของเรา กลับไปใช้ชื่อจาก eWeLink
```

ชื่อคือสิ่งที่จาร์วิสฟังเวลาสั่งด้วยเสียง ตั้งให้ไม่ซ้ำกันและพูดง่าย ("ไฟหน้าบ้าน" ดีกว่า "CH1")

**ทดสอบจาก VPS ผ่านด่านเดียวกับเสียงทุกด่าน:**

```bash
$B ewelink-switch …xxxx - on       # ปลั๊กหรือไฟดวงเดียวใช้ -
$B ewelink-switch …zzzz 1 on       # สวิตช์หลายช่องใส่เลขช่อง
$B ewelink-switch …zzzz 1 off
```

คำสั่งพิมพ์ผลจริงจาก eWeLink: `ok` / `offline` / `failed:<code>` / `refused:<เหตุผล>`

### ตั้งชื่อและสิทธิ์จากมือถือ (0.47.0)

ตั้งแต่ 0.47.0 **ไม่ต้องรัน `ewelink-name` หรือ `ewelink-allow` บน VPS อีก**: แผงควบคุม → "ไฟในบ้าน"
ตั้งชื่อไฟและชื่อแต่ละช่อง แก้ ลบ และติ๊ก "อนุญาตให้สั่ง" ทีละดวง ทุกอย่างยังเก็บในไฟล์เดิมบน VPS
(`ewelink_names.json`, `ewelink_allowlist.json`) คำสั่ง CLI ยังใช้ได้เหมือนเดิม

- ชื่อที่ตั้งเองใช้แทนชื่อจาก eWeLink เสมอ และสั่งด้วยเสียงได้ทันที
- ชื่อที่จาร์วิสแยกไม่ออกจากชื่ออื่นถูกปฏิเสธ ("ไฟหน้าบ้าน" กับ "หน้าบ้าน" นับว่าซ้ำ)
- สวิตช์ฉุกเฉินอยู่บน VPS เท่านั้น (ด้านล่าง) มือถือเปิดคืนเองไม่ได้
- log: `home name via=screen target=…aa01:1 set=yes|removed` และ `home allow via=screen target=…aa01:1 allowed=yes|no`
  ไม่มีชื่อ ไม่มี id เต็ม

**จำนวนช่อง:** Switch1 เป็น uiid 8 = "Three-channel Switch" ตาม UIIDProtocol.md แต่ส่ง `switches` มา 4 ตัว
broker ตัดช่องที่ไม่มีจริงทิ้งตาม uiid (2/3/4/7/8/9) uiid อื่นที่เอกสารไม่บอกจำนวน ช่องที่ไม่มีชื่อถือว่ายังไม่ใช้งาน
ต้องตั้งชื่อก่อนจึงจะสั่งได้

### 🔴 หยุดการสั่งทั้งหมดทันที (สวิตช์ฉุกเฉิน)

```bash
$B ewelink-control off     # ทุกคำสั่งเสียงและทุกการกดถูกปฏิเสธตั้งแต่วินาทีนี้ การ์ดกลับเป็นอ่านอย่างเดียวภายในรอบถัดไป
$B ewelink-control status
$B ewelink-control on      # กลับมาสั่งได้ (เฉพาะ allowlist)
```

ไม่ต้อง restart broker ไม่ลบ token และการ์ดยังแสดงสถานะไฟตามเดิม ถ้าจะตัดทั้งหมดใช้ `ewelink-disconnect`
เอาอุปกรณ์ออกจาก allowlist ทีละตัว: `$B ewelink-deny …xxxx`

**log ของการสั่ง** (`journalctl -u kiosk-broker`): `home switch via=voice|screen|vps target=…aa01:1 on=on result=ok`
มีแค่ id 4 ตัวท้ายกับเลขช่อง ไม่มี id เต็ม ไม่มีชื่อ ไม่มี token
...
ตั้งแต่ 0.46.0 คนที่ได้เครื่องไปสั่งเปิดปิดไฟใน allowlist ผ่าน broker ได้ จนกว่าจะทำข้อ 1:
1. ตัดมือถือออกจาก broker ทันที: `$B ewelink-control off` ก่อน (หยุดการสั่งไฟทันที) แล้ว
   `$B list-devices` และ `$B revoke-token <label>`
   (มือถือเครื่องนั้นเรียก broker ไม่ได้อีก การ์ดบ้านและทุกอย่างหยุด) จากนั้น `$B ewelink-control on`
...
- โควตา App ID ฟรี: 50,000 ครั้งต่อเดือนต่อภูมิภาค broker หยุดเองที่ 40,000 การ์ดบนจอใช้ราว
  ...
  การสั่งไฟหนึ่งครั้งใช้ 1 ครั้ง (หลายอุปกรณ์รวมเป็น batch ละไม่เกิน 10)
- 🔶 สวิตช์หลายช่อง: broker ส่งเฉพาะช่องที่สั่ง `{"switches":[{"switch":"on","outlet":N}]}`
  ตัวอย่างในเอกสารส่งครบทุกช่อง ซึ่งจะไปเปลี่ยนช่องอื่นด้วย **ต้องทดสอบบนเครื่องจริง:** สั่งช่อง 1
  แล้วดูว่าช่อง 2 ถึง 4 ไม่เปลี่ยน
- 🔶 ชื่อช่องมาจาก `tags.ck_channel_name` (key ที่แอป eWeLink ใช้ ยืนยันจาก homebridge-ewelink และ
  ewelink-api ไม่ใช่จากเอกสารทางการ) ถ้าไม่มี ใช้ `ewelink-name`
...
## สถานีตรวจวัด — ค่าที่วัดจริง (ไม่ต้องสมัคร key แล้ว, 2026-09-26)

เดิมหัวข้อนี้ให้สมัคร uid/ukey ของ data.tmd.go.th/api/ (TMDAPI) แต่ Poom สมัครไม่ได้ **เลิกใช้ TMDAPI แบบถาวรแล้ว**
ค่าที่วัดจริง (อุณหภูมิ/ความชื้น/ฝน) ตอนนี้มาจากแหล่งฟรีที่ไม่ต้องสมัครแทน — เลือกอัตโนมัติจากสถานีที่ใกล้ตำแหน่งปัจจุบัน
ของตู้ที่สุด ไม่ต้องตั้งค่าอะไรเพิ่ม:

- **SYNOP** — สถานีตรวจอากาศผิวพื้นของกรมอุตุฯ (ผ่านเครือข่ายสาธารณะ ไม่ใช่ data.tmd.go.th/api/)
- **METAR** — สถานีสนามบิน
- **สสน.** (สถาบันสารสนเทศทรัพยากรน้ำ) — สถานีวัดน้ำ/ฝน
- **Air4Thai** — สถานีคุณภาพอากาศ (ใช้ค่าฝนเสริมได้)

ถ้าไฟล์ env เก่ายังมี `TMD_UID`/`TMD_UKEY` ค้างอยู่ (ค่าที่ไม่มีอะไรอ่านแล้ว) ลบทิ้งได้ด้วย:
```
$B keys unset tmd    # มีถามยืนยัน y/N — ลบ TMD_UID และ TMD_UKEY ทั้งสองบรรทัดออกจากไฟล์
```
`$B keys` จะเตือนบรรทัดเดียวถ้าเห็นค่าเก่าคู่นี้ยังอยู่ในไฟล์


## พยากรณ์กรมอุตุฯ แบบ NWP (data.tmd.go.th/nwpapi) — ใส่ key (0.69, สำรวจก่อน ยังไม่ต่อเข้าการ์อากาศ)

คนละผลิตภัณฑ์กับสถานีตรวจวัดด้านบน: อันนี้เป็น**แบบจำลองพยากรณ์**ตามพิกัด/จังหวัด/ภาค (ราย 3 ชม. ล่วงหน้า 10 วัน
ความละเอียด 9 กม. หรือรายชั่วโมงล่วงหน้า 72 ชม. ความละเอียด 3 กม.) ไม่มีค่าที่วัดจริงจากสถานีเลย — ค่าที่วัดจริงมาจาก
แหล่งฟรีไม่ต้องสมัครในหัวข้อด้านบนแทน key ตัวนี้เป็น **JWT ตัวเดียว** (ขึ้นต้นด้วย `eyJ`) ส่งเป็น
...
```
$B probe tmd      # สำรวจเฉพาะ NWP (data.tmd.go.th/nwpapi) — ไม่มี key tmd-nwp ก็ข้าม ไม่ยิง request
$B probe gistda
```
